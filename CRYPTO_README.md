...
對應 OKX 上線中的 USDT 線性永續合約；排除重複代號，並檢查跨來源價格差。
符號與價格交叉檢查仍不等同合約地址身分驗證，交易前應確認資產身分。
無對應合約不分析，報告明列 200 名、對應數、完成分析數與排除統計。
全市場成交額至少 2,000 萬美元；OKX 最近 24 根已收 1H K 線成交額至少 1,000 萬 USDT。
依 OKX API 第八欄 volCcyQuote 計算 USDT 成交額，而非把合約張數當美元。
僅採用 confirm=1 的完整 1H 和 4H K 線，最少 80 根，拒絕缺漏、重複、過期資料。
...
- 持倉量只顯示 OKX 美元快照，不推論多空或增減；未實作歷史 OI 變化。
